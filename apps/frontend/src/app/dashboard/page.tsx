...
export default function DashboardPage() {
  ...
  const filteredWebsites = useMemo(() => {
    return websites.filter((w) => {
      const matchesSearch = w.url.toLowerCase().includes(searchQuery.toLowerCase());
      if (filterStatus === "all") return matchesSearch;
      const isUp = w.ticks.length === 0 || w.ticks[w.ticks.length - 1]?.status === "Good";
      ...
      <div className="min-h-[calc(100vh-4rem)] bg-background">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
            <StatCard title="Total Monitors" value={stats.total} icon={<Activity className="h-5 w-5 text-purple-500" />} accentColor="purple" />
            <StatCard title="Operational" value={stats.operational} icon={<Globe className="h-5 w-5 text-emerald-500" />} accentColor="emerald" />
            <StatCard title="Down" value={stats.down} icon={<PauseCircle className="h-5 w-5 text-rose-500" />} accentColor="rose" />
          </div>

          {/* Filters */}
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
            <h2 className="text-xl font-semibold tracking-tight">Your Monitors</h2>
            <div className="flex gap-3">
              <div className="relative flex-1 sm:min-w-[240px]">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input placeholder="Search monitors..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
              </div>
              <div className="flex gap-1 rounded-lg border border-border p-1 bg-muted/30">
                <FilterBtn active={filterStatus === "all"} onClick={() => setFilterStatus("all")}>All</FilterBtn>
                <FilterBtn active={filterStatus === "up"} onClick={() => setFilterStatus("up")}>Up</FilterBtn>
                <FilterBtn active={filterStatus === "down"} onClick={() => setFilterStatus("down")}>Down</FilterBtn>
              </div>
            </div>
          </div>
          ...
          <p className="text-xs text-muted-foreground text-right mb-4">
            Uptime over recent checks. Each bar = 1 check.
          </p>

          {/* Monitor List — Status Page Style */}
          {filteredWebsites.length === 0 ? (
            ...
            <div className="space-y-4">
              {filteredWebsites.map((website) => (
                <MonitorRow key={website.id} website={website} onDelete={handleDeleteMonitor} />
              ))}
            </div>
          )}
        </div>
...
function MonitorRow({
  ...
  website: {
    id: string;
    url: string;
    ticks: { id: string; status: string; latency: number; createdAt: string; updatedAt: string }[];
  };
  ...
  const uptimePercent = useMemo(() => {
    if (website.ticks.length === 0) return "0.00";
    const good = website.ticks.filter((t) => t.status === "Good").length;
    return ((good / website.ticks.length) * 100).toFixed(2);
  }, [website.ticks]);

  // Get all ticks for the bar visualization
  const allTicks = website.ticks;

  // Time labels
  const oldestTick = allTicks.length > 0 ? allTicks[0] : null;
  const newestTick = allTicks.length > 0 ? allTicks[allTicks.length - 1] : null;
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-5 hover:border-purple-500/30 transition-all duration-300 group">
      {/* Header: Name + Status */}
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className="relative flex-shrink-0">
            <div className={`w-2.5 h-2.5 rounded-full ${isUp ? "bg-emerald-500" : "bg-rose-500"}`} />
            <div className={`absolute inset-0 w-2.5 h-2.5 rounded-full animate-ping ${isUp ? "bg-emerald-500/40" : "bg-rose-500/40"}`} />
          </div>
          <div className="min-w-0">
            <h3 className="font-semibold text-sm truncate">{extractDomain(website.url)}</h3>
      ...
      {/* ── Tick Bar Visualization (Status Page Style) ── */}
      <div className="mb-3">
      ...
      {/* Time range labels + uptime % */}
      <div className="flex items-center justify-between text-[11px] text-muted-foreground mb-4">
        <span>{oldestTick ? formatTimeAgo(oldestTick.createdAt) : "—"}</span>
        <div className="flex-1 flex items-center justify-center gap-2">
          <div className="h-[1px] flex-1 bg-border" />
          <span className="font-medium text-foreground/70">{uptimePercent} % uptime</span>
          <div className="h-[1px] flex-1 bg-border" />
        </div>
        <span>{newestTick ? "Now" : "—"}</span>
      </div>
      ...
      <div className="flex items-center gap-6 text-xs text-muted-foreground">
        <div className="flex items-center gap-1.5">
          ...
          <span>Uptime: <span className="font-semibold text-foreground">{uptimePercent}%</span></span>
        </div>
        <div className="flex items-center gap-1.5">
          <Clock className="h-3 w-3" />
          <span>Checks: <span className="font-semibold text-foreground">{website.ticks.length}</span></span>
        </div>
        ...
        <button
          className="flex items-center gap-1 text-muted-foreground hover:text-rose-500 transition-colors cursor-pointer"
...
function StatCard({
  ...
  title: string; value: number; icon: React.ReactNode; accentColor: "purple" | "emerald" | "rose";
}) {
  const glowMap = {
  ...
  return (
    <div className={`relative overflow-hidden rounded-xl border bg-gradient-to-br ${glowMap[accentColor]} transition-colors duration-300 group p-6 flex items-center justify-between`}>
      <div>
